            ...
            <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                <div className="flex h-16 items-center justify-between">
                    ...
                    <div className="hidden items-center gap-6 md:flex">
                        ...
                        {isAuth && (
                            <>
                                {/* My Pets / My Products только для user — не для admin/manager */}
                                {user?.role === "user" && (
                                    <>
                                ...
                                {(user?.role === "manager" || user?.role === "admin") && (
                                    <Link to="/manager" className="text-sm font-medium text-muted-foreground transition-colors hover:text-primary">
                                        {t("nav.manager")}
                                    </Link>
                                )}
                            </>
                ...
                {mobileMenuOpen && (
                    <div className="border-t border-border py-4 md:hidden">
                        <div className="flex flex-col gap-4">
                            ...
                            {isAuth && (
                                <>
                                    {/* My Pets / My Products только для user */}
                                    {user?.role === "user" && (
                                        <>
                                    ...
                                    {/* Admin/Manager links in mobile too */}
                                    {user?.role === "admin" && (
                                    ...
                                    {(user?.role === "manager" || user?.role === "admin") && (
                                        <Link to="/manager" className="text-sm font-medium text-muted-foreground">
                                            {t("nav.manager")}
                                        </Link>
                                    )}
                                </>
